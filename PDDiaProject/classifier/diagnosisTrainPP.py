...
class NiiSliceDataset(Dataset):
    ...
    def __getitem__(self, index):
        slice_img, label, name, z = self.slice_samples[index]

        slice_img = np.expand_dims(slice_img, axis=0)

        return (
            torch.tensor(slice_img, dtype=torch.float32),
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for images, labels, names, z in loader:
...
def evaluate_slice_level(model, loader, criterion, device):
    ...
    不做 patient_prob = mean(slice_probs)
    不做病人级聚合

    输出:
        slice级 AUC/ACC/SEN/SPE/PRE/F1
        每个病人的 slice_probs，供后续 Viterbi 使用
    """
    ...
    with torch.no_grad():
        for volume, label, name in loader:
            volume = volume.to(device)
            label = label.to(device)

            B, Z, C, H, W = volume.shape

            # [B,Z,C,H,W] -> [B*Z,C,H,W]
            volume = volume.reshape(B * Z, C, H, W)

            logits = model(volume)

            # 病人标签复制给该病人的每一张 slice
            ...
            # 当前代码 val/test 的 batch_size=1，所以这里对应一个病人
...
def main():
    ...
    train_root = "./data/qsm_train"
    test_root = "./data/qsm_test"
    ...
    for fold, (train_idx, val_idx) in enumerate(
        ...
        train_dataset = NiiSliceDataset(
            fold_train_samples
        )

        val_dataset = NiiPatientDataset(
            fold_val_samples
        )

        test_dataset = NiiPatientDataset(
            test_samples
        )
    ...
    print(f"平均 Val Slice ACC : {fold_metrics_np[:, 2].mean():.4f}")
    print(f"平均 Val Slice SEN : {fold_metrics_np[:, 3].mean():.4f}")
    print(f"平均 Val Slice SPE : {fold_metrics_np[:, 4].mean():.4f}")
    print(f"平均 Val Slice PRE : {fold_metrics_np[:, 5].mean():.4f}")
    print(f"平均 Val Slice F1  : {fold_metrics_np[:, 6].mean():.4f}")

    print(f"平均 Test Slice AUC: {fold_metrics_np[:, 7].mean():.4f}")
    print(f"平均 Test Slice ACC: {fold_metrics_np[:, 8].mean():.4f}")
    print(f"平均 Test Slice SEN: {fold_metrics_np[:, 9].mean():.4f}")
    print(f"平均 Test Slice SPE: {fold_metrics_np[:, 10].mean():.4f}")
    print(f"平均 Test Slice PRE: {fold_metrics_np[:, 11].mean():.4f}")
    print(f"平均 Test Slice F1 : {fold_metrics_np[:, 12].mean():.4f}")
