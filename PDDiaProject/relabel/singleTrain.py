...
from relabel.singleFeatureFrozenNet import SingleFeatureFrozenNet
...
def set_trainable_stage(model, stage):
    """
    stage=1:
        训练 FeatureExtraction + ClassifierMLP

    stage=2:
        冻结 FeatureExtraction，只训练 ClassifierMLP
    """
...
def train_stage(
    ...
    best_model_path = os.path.join(
        save_dir,
        f"best_fold_{fold}_stage_{stage}.pth"
    )
    ...
    for epoch in range(num_epochs):
        ...
        if early_stopping.early_stop:
            print(f"Fold {fold} Stage {stage} 早停止")
            break
...
def main():
    ...
    save_dir = "./result/single_feature_frozen"
    ...
    for fold, (train_idx, val_idx) in enumerate(
        ...
        model = SingleFeatureFrozenNet().to(device)

        print("\n----- Stage 1: 训练 FeatureExtraction + MLP -----")

        model, best_stage1_path = train_stage(
            model=model,
        ...
        print("\n----- Stage 2: 冻结 FeatureExtraction，只训练 MLP -----")

        model, best_stage2_path = train_stage(
            model=model,
            train_loader=train_loader,
            val_loader=val_loader,
            stage=2,
